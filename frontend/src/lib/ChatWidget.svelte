<script lang="ts">
  ...
  onMount(async () => {
    ...
    if (savedSession) {
      ...
      try {
        const historyMessages = await fetch(`http://localhost:3001/chat/history/${savedSession}`)
          .then(res => {
            if (!res.ok) throw new Error('Failed to fetch history');
            return res.json();
          });
        
        // 3. Map backend response to UI format
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Network pipeline exception');
      ...
      // 4. Push real AI generated response to UI
      messages.push({
        sender: 'AI',
        text: data.reply,
        timestamp: new Date()
...
<div class="w-full h-full bg-white flex flex-col overflow-hidden">
  ...
  >
    ...
    <!-- Dynamic Agent is Typing CSS Core Pulse Animation -->
    {#if isAiTyping}
  ...
  <!-- Defensive Interactive Input Bar Layer -->
